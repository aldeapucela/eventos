...
const WEATHER_CACHE_KEY = 'aldeapucela-eventos-weather-v1';
...
export async function loadWeatherForecast() {
  ...
  weatherPromise = fetchForecast().finally(() => {
    weatherPromise = null;
  });
...
async function fetchForecast() {
  ...
  try {
    const response = await window.fetch(WEATHER_API_URL, {
      headers: { Accept: 'application/json' },
      signal: controller.signal
    });
    if (!response.ok) throw new Error(`Weather request failed with status ${response.status}`);
    const days = normalizeWeatherPayload(await response.json());
...
export function getWeatherLabel(day) {
  ...
  if (Number.isFinite(day.min) && Number.isFinite(day.max)) {
...
function normalizeWeatherPayload(payload) {
  ...
  const rainMaximums = Array.isArray(daily?.precipitation_probability_max) ? daily.precipitation_probability_max : [];
  ...
  hourlyTimes.forEach((timestamp, index) => {
    ...
    hourlyByDate[date].push({
      ...
      weatherCode,
      temperature,
      rainProbability: toFiniteNumber(hourlyRain[index])
    });
  });
  ...
  dates.forEach((date, index) => {
    ...
    days[date] = {
      ...
      rainProbability: toFiniteNumber(rainMaximums[index]),
