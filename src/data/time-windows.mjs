...
// ponytail: self-check (ventanas coherentes y cada página con su encabezado propio).
if (process.argv[1] && (await import('node:url')).fileURLToPath(import.meta.url) === process.argv[1]) {
  ...
  const sinInterpolar = pages.filter((page) => page.h2.includes('${') || page.description.includes('${'));
  console.assert(!sinInterpolar.length, `h2 o description sin interpolar: ${sinInterpolar.map((p) => p.slug).join(', ')}`);
  // El finde empieza el viernes por la tarde.
  ...
  console.log(`ok: ${pages.length} páginas temporales`);
}
